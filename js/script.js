...
const config = {
    ...
    */
    contactPage: {
        email: "" /*E-Mail-Adresse für Kontaktformular - NOCH NICHT VORHANDEN - BITTE SPÄTER EINTRAGEN*/
    }
}
...
const getDiscordOnlineUsers = async () => {
    try {
        ...
        if (!response.ok) {
            console.log("Discord Invite API Error:", response.status, response.statusText);
            return "0";
        }

        const data = await response.json();

        // ✅ KORREKTER Wert aus deiner API
        if (typeof data.approximate_presence_count === "number") {
            return data.approximate_presence_count.toString();
        }

        // Fallback: falls Discord das Feld mal nicht liefert
        return "0";

    } catch (e) {
        console.log("Discord Invite API Fetch Error:", e);
        return "0";
    }
};
...
const getSkinByUuid = async (username) => {
    try {
        // Verwende minotar.net API für Bust-Bilder (512x512)
        const skinApi = `https://minotar.net/bust/${username}/512.png`;
        let response = await fetch(skinApi);

        if(response.ok) {
            return skinApi;
        } else {
            // Fallback auf Standard-Skin falls Fehler
            return `https://minotar.net/bust/Steve/512.png`;
        }
    } catch (e) {
        console.log("Skin Error:", e);
        // Fallback auf Standard-Skin
        return `https://minotar.net/bust/Steve/512.png`;
    }
}
...
const setDataFromConfigToHtml = async () => {
    ...
    if(locationPathname == "/" || locationPathname.includes("index")) {
    ...
    else if(locationPathname.includes("admin-team") || locationPathname.includes("team")) {
        // Mapping für bessere Gruppentitel
        const groupTitleMap = {
            "leaders": "Führung",
        ...
        for (let team in config.adminTeamPage) {
            ...
            const groupTitle = groupTitleMap[team] || (team.charAt(0).toUpperCase() + team.slice(1).replace(/([A-Z])/g, ' $1').trim());
            
            const groupSchema = `
                <h2 class="rank-title">${groupTitle}</h2>
                <div class="users">
                </div>
            `;
