...
async function resolveUsername(_env, username) {
  try {
    ...
    console.error("[Roblox] username lookup threw:", e.message);
    return { type: "apiError", status: 500, message: e.message, debugUrl: "v1/usernames/users" };
  }
}
...
var RobloxController = class {
  ...
  static async getPlayer(request, env2, user, params) {
    ...
    try {
      ...
      let profile3;
      try {
        const profileRes = await cloudFetch(env2, `https://apis.roblox.com/cloud/v2/users/${userId}`);
        if (profileRes.ok) {
...
var CloudController = class {
   ...
   */
  static async shutdownServer(request, env2, user) {
    const origin = env2.ALLOWED_ORIGIN ?? "https://bwrp.net";
    ...
    try {
      ...
      await auditLog(env2.DATABASE, Number(user.sub), "SERVER_SHUTDOWN", "servers", serverJobId, { serverJobId }, getIP(request));
      new DiscordService(env2).sendSystemNotification?.(`${user.username} hat Server ${serverJobId.slice(0, 8)}... heruntergefahren.`).catch(() => {
      });
   ...
   */
  static async restartAll(request, env2, user) {
    const origin = env2.ALLOWED_ORIGIN ?? "https://bwrp.net";
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Nur ADMIN+ kann alle Server neustarten", 403, origin);
    const body = await request.json().catch(() => ({}));
    const extraProtected = Array.isArray(body.extraProtectedJobIds) ? body.extraProtectedJobIds : [];
    const envProtected = env2.PROTECTED_JOB_IDS ? env2.PROTECTED_JOB_IDS.split(",").map((s) => s.trim()).filter(Boolean) : [];
    const allProtected = [.../* @__PURE__ */ new Set([...envProtected, ...extraProtected])];
    try {
      const cloud = new RobloxCloudService(env2);
      await cloud.publishMessage("StaffPanelUpdates", {
        type: "RESTART_ALL",
        excludeJobIds: allProtected,
        // In-game script checks: if excludeJobIds.includes(game.JobId) → skip
        issuedBy: user.username,
        issuedAt: (/* @__PURE__ */ new Date()).toISOString()
      });
      await auditLog(env2.DATABASE, Number(user.sub), "SERVER_RESTART_ALL", "servers", void 0, { protectedCount: allProtected.length }, getIP(request));
      new DiscordService(env2).sendSystemNotification?.(`${user.username} hat RESTART ALL ausgel\xF6st. ${allProtected.length} Server gesch\xFCtzt.`).catch(() => {
      });
      return json({
        success: true,
        message: `Restart-Signal gesendet. ${allProtected.length} Server sind gesch\xFCtzt.`,
...
var ManagementController = class _ManagementController {
  ...
  static async listStaff(request, env2, user) {
    ...
    `).all();
    const roleRows = await env2.DATABASE.prepare(`
  ...
  static async getUserActivity(request, env2, user, params) {
    const origin = env2.ALLOWED_ORIGIN ?? "https://bwrp.net";
    const bad = _ManagementController.checkAccess(user, origin);
    if (bad) return bad;
    const targetId = parseInt(params.id);
    if (isNaN(targetId) || targetId <= 0) return err("Ung\xFCltige User-ID", 400, origin);
    const targetUser = await env2.DATABASE.prepare(
    ...
    const logs = await env2.DATABASE.prepare(`
      ...
      LIMIT 20
    `).bind(targetId).all();
    const shiftStats = await env2.DATABASE.prepare(`
    ...
    `).bind(targetId).first();
    const caseStats = await env2.DATABASE.prepare(`
      ...
      WHERE moderator_id = ?
    `).bind(targetId).first();
    const customRoles = await env2.DATABASE.prepare(`
    ...
    return json({
      ...
      recentActivity: logs.results.map((l) => ({
        action: l.action,
        resource: l.resource,
...
init_auth();
init_types();
var RolesController = class _RolesController {
  static {
    __name(this, "RolesController");
  }
  static origin(env2) {
    return env2.ALLOWED_ORIGIN ?? "https://bwrp.net";
  }
  // ─── GET /api/roles/permissions ──────────────────────────────────────────────
  static async listPermissions(_req, env2, user) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Zugriff verweigert", 403, o);
    return json({ permissions: ALL_PERMISSIONS }, 200, o);
  }
  // ─── GET /api/roles ───────────────────────────────────────────────────────────
  static async listRoles(_req, env2, user) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Zugriff verweigert", 403, o);
    const rows = await env2.DATABASE.prepare(
  ...
  // ─── POST /api/roles ──────────────────────────────────────────────────────────
  static async createRole(request, env2, user) {
    const o = _RolesController.origin(env2);
    ...
    if (typeof hierarchy !== "number" || hierarchy < 0 || hierarchy > 1e3) {
    ...
    if (!Array.isArray(permissions) || permissions.some((p) => !ALL_PERMISSIONS.includes(p))) {
      return err("Ung\xFCltige Berechtigungen", 400, o);
    }
    if (!/^#[0-9a-fA-F]{6}$/.test(color)) {
      return err("Ung\xFCltige Farbe (erwartet #RRGGBB)", 400, o);
    }
    const existing = await env2.DATABASE.prepare("SELECT id FROM roles WHERE name = ?").bind(name.trim()).first();
    if (existing) return err(`Rolle "${name}" existiert bereits`, 409, o);
    const result = await env2.DATABASE.prepare(
      "INSERT INTO roles (name, color, hierarchy, permissions) VALUES (?, ?, ?, ?)"
    ).bind(name.trim(), color, hierarchy, JSON.stringify(permissions)).run();
    await auditLog(env2.DATABASE, Number(user.sub), "ROLE_CREATE", "roles", String(result.meta.last_row_id), { name: name.trim(), hierarchy }, getIP(request));
  ...
  static async updateRole(request, env2, user, params) {
    ...
    if (isNaN(roleId) || roleId <= 0) return err("Ung\xFCltige Rollen-ID", 400, o);
    const existing = await env2.DATABASE.prepare("SELECT * FROM roles WHERE id = ?").bind(roleId).first();
    if (!existing) return err("Rolle nicht gefunden", 404, o);
    const body = await request.json().catch(() => ({}));
    const name = body.name !== void 0 ? String(body.name).trim() : existing.name;
    const color = body.color !== void 0 ? String(body.color) : existing.color;
    const hierarchy = body.hierarchy !== void 0 ? Number(body.hierarchy) : existing.hierarchy;
    const permissions = body.permissions !== void 0 ? body.permissions : JSON.parse(existing.permissions ?? "[]");
    if (name.length < 2) return err("Rollenname muss mindestens 2 Zeichen haben", 400, o);
    if (isNaN(hierarchy) || hierarchy < 0 || hierarchy > 1e3) return err("Hierarchie muss zwischen 0 und 1000 liegen", 400, o);
    if (!Array.isArray(permissions) || permissions.some((p) => !ALL_PERMISSIONS.includes(p))) return err("Ung\xFCltige Berechtigungen", 400, o);
    if (!/^#[0-9a-fA-F]{6}$/.test(color)) return err("Ung\xFCltige Farbe (erwartet #RRGGBB)", 400, o);
    if (name !== existing.name) {
      const conflict = await env2.DATABASE.prepare("SELECT id FROM roles WHERE name = ? AND id != ?").bind(name, roleId).first();
      if (conflict) return err(`Rolle "${name}" existiert bereits`, 409, o);
    }
    await env2.DATABASE.prepare(
    ...
    await auditLog(env2.DATABASE, Number(user.sub), "ROLE_UPDATE", "roles", String(roleId), { name, hierarchy }, getIP(request));
    return json({ success: true }, 200, o);
  }
  // ─── DELETE /api/roles/:id ────────────────────────────────────────────────────
  static async deleteRole(request, env2, user, params) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["OWNER"]) return err("Nur OWNER kann Rollen l\xF6schen", 403, o);
    const roleId = parseInt(params.id);
    if (isNaN(roleId) || roleId <= 0) return err("Ung\xFCltige Rollen-ID", 400, o);
    const existing = await env2.DATABASE.prepare("SELECT name FROM roles WHERE id = ?").bind(roleId).first();
    if (!existing) return err("Rolle nicht gefunden", 404, o);
    await env2.DATABASE.prepare("DELETE FROM roles WHERE id = ?").bind(roleId).run();
    await auditLog(env2.DATABASE, Number(user.sub), "ROLE_DELETE", "roles", String(roleId), { name: existing.name }, getIP(request));
    return json({ success: true }, 200, o);
  }
  // ─── GET /api/roles/users/:userId ────────────────────────────────────────────
  static async getUserRoles(_req, env2, user, params) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Zugriff verweigert", 403, o);
    const userId = parseInt(params.userId);
    if (isNaN(userId) || userId <= 0) return err("Ung\xFCltige User-ID", 400, o);
    const rows = await env2.DATABASE.prepare(`
  ...
  static async assignRole(request, env2, user, params) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Zugriff verweigert", 403, o);
    const userId = parseInt(params.userId);
    if (isNaN(userId) || userId <= 0) return err("Ung\xFCltige User-ID", 400, o);
    ...
    if (isNaN(roleId) || roleId <= 0) return err("Ung\xFCltige Rollen-ID", 400, o);
    const targetUser = await env2.DATABASE.prepare("SELECT id, username FROM users WHERE id = ?").bind(userId).first();
    if (!targetUser) return err("User nicht gefunden", 404, o);
    const role = await env2.DATABASE.prepare("SELECT id, name FROM roles WHERE id = ?").bind(roleId).first();
    ...
    await auditLog(env2.DATABASE, Number(user.sub), "ROLE_ASSIGN", "user_roles", `${userId}:${roleId}`, { username: targetUser.username, roleName: role.name }, getIP(request));
    return json({ success: true }, 200, o);
  }
  // ─── DELETE /api/roles/users/:userId/:roleId ─────────────────────────────────
  static async removeRole(request, env2, user, params) {
    const o = _RolesController.origin(env2);
    if (ROLE_RANK[user.role] < ROLE_RANK["ADMIN"]) return err("Zugriff verweigert", 403, o);
    const userId = parseInt(params.userId);
    const roleId = parseInt(params.roleId);
    if (isNaN(userId) || isNaN(roleId)) return err("Ung\xFCltige ID", 400, o);
    await env2.DATABASE.prepare(
      "DELETE FROM user_roles WHERE user_id = ? AND role_id = ?"
    ).bind(userId, roleId).run();
    await auditLog(env2.DATABASE, Number(user.sub), "ROLE_REMOVE", "user_roles", `${userId}:${roleId}`, {}, getIP(request));
    return json({ success: true }, 200, o);
  }
};
...
init_auth();
init_types();
var NotesController = class _NotesController {
  static {
    __name(this, "NotesController");
  }
  static origin(env2) {
    return env2.ALLOWED_ORIGIN ?? "https://bwrp.net";
  }
  /** Ensure a note row exists for this user and return it. */
  static async getOrCreate(env2, userId) {
    const existing = await env2.DATABASE.prepare(
    ...
    return { id: 0, user_id: userId, content: "", pinned_tickets: "[]", updated_at: (/* @__PURE__ */ new Date()).toISOString() };
  }
  // ─── GET /api/notes ───────────────────────────────────────────────────────────
  static async getNote(_req, env2, user) {
    const o = _NotesController.origin(env2);
    const note = await _NotesController.getOrCreate(env2, Number(user.sub));
    return json({
  ...
  // ─── PUT /api/notes ───────────────────────────────────────────────────────────
  static async saveNote(request, env2, user) {
    const o = _NotesController.origin(env2);
    const body = await request.json().catch(() => ({}));
    if (typeof body.content !== "string") return err("content muss ein String sein", 400, o);
    if (body.content.length > 1e4) return err("Notiz darf maximal 10.000 Zeichen haben", 400, o);
    const userId = Number(user.sub);
    const note = await _NotesController.getOrCreate(env2, userId);
    await env2.DATABASE.prepare(
   ...
   */
  static async pinTicket(request, env2, user) {
    const o = _NotesController.origin(env2);
    ...
    const userId = Number(user.sub);
    const note = await _NotesController.getOrCreate(env2, userId);
    const pins = JSON.parse(note.pinned_tickets ?? "[]");
    if (pins.some((p) => p.incidentId === incidentId)) {
    ...
    pins.unshift({ caseId, incidentId, type, targetUsername, createdAt: createdAt ?? (/* @__PURE__ */ new Date()).toISOString() });
    await env2.DATABASE.prepare(
  ...
  static async unpinTicket(request, env2, user) {
    const o = _NotesController.origin(env2);
    const body = await request.json().catch(() => ({}));
    ...
    const userId = Number(user.sub);
    const note = await _NotesController.getOrCreate(env2, userId);
    const pins = JSON.parse(note.pinned_tickets ?? "[]").filter((p) => p.incidentId !== incidentId);
    await env2.DATABASE.prepare(
      "UPDATE notes SET pinned_tickets = ?, updated_at = datetime('now') WHERE user_id = ?"
    ).bind(JSON.stringify(pins), userId).run();
    return json({ success: true, pinnedTickets: pins }, 200, o);
...
var ROUTES = [
  ...
  route("GET", "/api/mgmt/users/:id/activity", ManagementController.getUserActivity),
  // ── Dynamic Roles (OWNER CRUD, ADMIN read/assign) ─────────────────────────
  route("GET", "/api/roles/permissions", RolesController.listPermissions),
  route("GET", "/api/roles", RolesController.listRoles),
  route("POST", "/api/roles", RolesController.createRole),
  route("PATCH", "/api/roles/:id", RolesController.updateRole),
  route("DELETE", "/api/roles/:id", RolesController.deleteRole),
  route("GET", "/api/roles/users/:userId", RolesController.getUserRoles),
  route("POST", "/api/roles/users/:userId/assign", RolesController.assignRole),
  route("DELETE", "/api/roles/users/:userId/:roleId", RolesController.removeRole),
  // ── Personal Notes ────────────────────────────────────────────────────────
  route("GET", "/api/notes", NotesController.getNote),
  route("PUT", "/api/notes", NotesController.saveNote),
  route("POST", "/api/notes/pin", NotesController.pinTicket),
  route("POST", "/api/notes/unpin", NotesController.unpinTicket),
  // ── Server Power Operations (ADMIN+) ─────────────────────────────────────
  route("POST", "/api/cloud/servers/shutdown", CloudController.shutdownServer),
  route("POST", "/api/cloud/servers/restart-all", CloudController.restartAll),
